...
// Crea un espacio de Google Meet nuevo e independiente (uno por cita
// virtual). A propósito NO se pega como conferenceData del evento de
// Calendar — se pega como texto plano en la ubicación — así el paciente
// sigue recibiendo la invitación por correo (sigue siendo invitado del
// evento) pero no queda "reconocido" por Meet como invitado de esa
// reunión específica, y tiene que tocar la puerta para que la psicóloga
// lo admita.
export async function createMeetSpace() {
  const auth = await getAuthenticatedClient()
  if (!auth) return null

  const meet = google.meet({ version: 'v2', auth: auth.client })
